...
class Sites extends CI_Controller {
	...
	*/
	...
	public function siteAjaxUpdate() {
        ...
        $this->form_validation->set_rules('siteSettings_remoteUrl', 'Domain', 'callback_checkStrDomain|required');
		...
		if ($this->form_validation->run() == FALSE) {
			...
			//$mypermen = $data['siteSettings_remoteUrl'];
    ...
    public function checkDomain() {
        $return['free'] = $this->sitemodel->checkDomain($_POST['domain'], $_POST['site_id']);
        echo json_encode( $return );
    }

    public function checkStrDomain($domain) {
        $res = $this->sitemodel->checkDomain($domain, $_POST['siteID']);
        if(!$res) {
            $this->form_validation->set_message('checkStrDomain', 'Занятый домен');
            return false;
        }
        $re = '/^[a-z\d]{1,20}\.oke\.su$/';
        if(!preg_match($re, $domain)) {
            $this->form_validation->set_message('checkStrDomain', 'Некорректный домен');
            return false;
        }
        return true;
    }
	...
	*/
	...
	public function analitic(){
		$this->load->model('analitics_m');
		$data['regData'] = $this->analitics_m->dataClear();
		$data['siteAll'] = $this->analitics_m->siteAll();
		$this->load->view('sites/analitics',$data);
	}

	public function tarif(){
		$this->load->view('sites/tarif');
	}

	public function prodvichenie(){
		$this->load->view('sites/prodvi');
	}
