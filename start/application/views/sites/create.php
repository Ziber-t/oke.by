...
<body>
    ...
    <div class="menu" id="menu">
      		...
      		<div class="main" id="main">
				...
				<?php if( isset($templates) ):?>
      		  	<h3><span class="fui-document"></span> Шаблоны</h3>
      		  	
      		  	<ul id="templatesUl" style="margin-bottom: 30px">
      		  		<li><a href="#" id="templ">Готовые шаблоны</a></li>
      		  	</ul>
				<?php endif;?>
        ...
        <div class="container">
        	...
        	<header class="clearfix" data-spy="affix" data-offset-top="60" data-offset-bottom="200">
        		...
        		<div class="btn-group" style="float: right;">           
        			<button class="btn btn-default btn-embossed"><span class="fui-gear"></span> Настройки</button>
				...
				$prtest = $prtest['user_id'];
					...
					$testius22 = $testOplata->result();
					//var_dump($testius22);
					$testius22 = $testius22[0];
					$testius22 = $testius22->opl;
					//var_dump($testius22);
					if($testius22 == 0){
				?>
					<a id="exportPage" data-toggle="modal" class="btn btn-info btn-embossed pull-right disabled actionButtons" style="margin-right: 1px" onclick="alert('Для экспорта сайта перейдите на платный тариф!');"><span class="fui-export"></span> <?php echo $this->lang->line('actionbuttons_export')?></a>
				<?php 		
					}else{
				?>
				...
				<a href="#" id="publishPage" class="btn btn-info btn-embossed pull-right disabled actionButtons" data-siteid="<?php echo $siteData['site']->sites_id;?>" <?php if( $siteData['site']->ftp_ok == 0 ):?>data-toggle="tooltip" disabled <?php endif;?> data-placement="bottom" title="Вы не сможете опубликовать сайт, так как не указали адресс сайта" style="margin-right: 1px"><span class="fui-upload"></span> <?php echo $this->lang->line('actionbuttons_publish')?> <span class="fui-alert text-danger" <?php if( $siteData['site']->ftp_ok == 1 ):?>style="display:none"<?php endif;?>></span></a>
        			...
        			<div id="pageList">
        				...
        				<?php else:?>
        					...
        					<ul id="page<?php echo $counter;?>" class="ui-sortable" style="display: <?php if($counter==1):?>block<?php else:?>none<?php endif;?>;">
        						...
        						<li class="element">
        							<iframe id="<?php echo "ui-id-".($c+10000);?>" frameborder="0" <?php if( $frame->frames_sandbox == 1 ):?>data-loaderfunction="<?php echo $frame->frames_loaderfunction;?>" data-sandbox="<?php $frame->sandboxID = rand(0, 999999999); echo $frame->sandboxID;?>"<?php endif;?> scrolling="0" src="<?php echo site_url('sites/getframe/'.$frame->frames_id)?>" data-height="<?php echo $frame->frames_height;?>" data-originalurl="<?php echo $frame->frames_original_url?>"></iframe>
        						</li>
        						<?php $c++;?>
        						<?php endforeach;?>
        					</ul>
    	      					      		...
    	      					      		<tr>
    	      					        		<th class="text-center" style="width: 30px;">
    	      					        			<label class="checkbox no-label toggle-all" for="checkbox-table-1">
    	      					        				<input type="checkbox" value="" id="checkbox-table-1" data-toggle="checkbox" class="toggleAll" checked>
    	      					        			</label>
    	      					        		</th>
    	      					      		...
    	      					      		<tr>
    	      					        		<td class="text-center" style="width: 30px;">
    	      					        			<label class="checkbox no-label">
    	      					        				<input type="checkbox" value="<?php echo $folder;?>" id="" data-type="asset" name="assetFolders[]" data-toggle="checkbox" checked>
    	      					        			</label>
    	      					        		</td>
    	      					        		<td>
    	      					        			<?php echo $folder;?>
	...
	<?php if( $siteData['site']->ftp_ok == 0 ){ ?>
								<script> 
										window.onload = function(){
											function gol(){
												/*$('#siteSettings').modal('show');*/
												$("#siteSettingsButton").click();
											}
											setTimeout(gol, 2000);
										} 
								</script>
    ...
    $(function(){
    	...
    	$('#pageList li iframe').each(function(){
			...
			resetButton = $('<button type="button" class="btn btn-warning resetBlock"><i class="fa fa-refresh"></i> сбросить</button>');
			htmlButton = $('<button type="button" class="btn btn-inverse htmlBlock"><i class="fa fa-code"></i> html код</button>');
		...
		setTimeout(function(){ $('.alert-success').each(function(){ $(this).fadeOut(500, function(){$(this).remove()}) }) }, 5000)
