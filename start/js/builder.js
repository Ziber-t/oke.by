...
$(document).ready(function() {
	...
	$('#second #templates iframe').each(function(){
...
pagesContents = {};//global used for saving page preivews on the server


function prepPagesforSave(page) {
		...
		if( $(this).find('iframe').size() > 0 ) {//we've got frames
			...
			$(this).find('iframe').each(function(){
				...
				if (typeof attr !== typeof undefined && attr !== false) {
					...
					frame['frameContent'] = "<html>"+$('#sandboxes #'+attr).contents().find('html').html()+"</html>";
					frame['frameSandbox'] = 1;
					frame['frameLoaderfunction'] = $(this).attr('data-loaderfunction');
									
				} else {
					
					frame['frameContent'] = "<html>"+$(this).contents().find('html').html()+"</html>";
					frame['frameSandbox'] = 0;
					frame['frameLoaderfunction'] = '';
			...
			if( page == '' || ( page != '' && page == $('#pages li:eq('+($(this).index()+1)+') a:first').text() ) ) {
			
				thePages[pageName] = pageFrames;
			
				pagesContents[pageName] = "<html>"+$('iframe#skeleton').contents().find('html').html()+"</html>";
	...
	el.sortable({
		...
		beforeStop: function(event, ui){
			...
			if (typeof attr !== typeof undefined && attr !== false) {//template, build it
				...
				$('#pageList ul#page1 li').each(function(){
				...
				for(x=0; x<frameIDs.length; x++) {
					...
					toInsert = $('<li><iframe src="'+siteUrl+'sites/getframe/'+frameIDs[x]+'" frameborder="0" scrolling="0" data-originalurl="'+urls[x]+'"></li>');
					
					$('#pageList #page1').append( toInsert );
				...
				if( ui.item.find('.frameCover > button').size() == 0 ) {
					...
					if( ui.item.find('iframe').size() > 0 ) {//iframe thumbnails
						...
						if (typeof attr !== typeof undefined && attr !== false) {
							...
							ui.item.html('<iframe src="'+ui.item.find('img').attr('data-srcc')+'" scrolling="no" data-originalurl="'+ui.item.find('img').attr('data-srcc')+'" frameborder="0" data-sandbox="'+theID+'" '+loaderFunction_+'><iframe>');
...
function setEditorEvents(theFrame){
			
	for( var key in editableItems ) {
										
		theFrame.contents().find( pageContainer + ' '+ key ).hover( function(){
			...
			if( $(this).closest('body').width() != $(this).width() ) {
				...
				$(this).css({'border': '3px dashed red', 'cursor': 'pointer'});
					
			}
			
		}, function(){
			
			//alert( $(this).closest('body').width() + " and " + $(this).width() )
			...
			if( $(this).closest('body').width() != ($(this).width()+6) ) {
				...
				$(this).css({'border': 'none', 'cursor': 'default'});
...
function styleClick(el) {
		...
		$('#styleEditor').on('click', 'button#removeElementButton', function(){
			...
			$('#deleteElement button#deleteElementConfirm').unbind('click').bind('click', function(){
				...
				toDel.fadeOut(500, function(){
					...
					$('#'+sandboxID).contents().find('#'+elementID).remove();
	...
	$('button#cloneElementButton').unbind('click').bind('click', function(){
		...
		if( $(el).parent().hasClass('propClone') ) {//clone the parent element
			...
			theClone2 = $(el).parent().clone();
			theClone2.find( $(el).prop('tagName') ).attr('style', '');
		...
		if( sandboxID ) {
			...
			$('#'+sandboxID).contents().find('#'+elementID).after( theClone2 )
	...
	$('button#resetStyleButton').unbind('click').bind('click', function(){
		...
		if( sandboxID ) {
			...
			$('#'+sandboxID).contents().find('#'+elementID).attr('style', '');
...
function setPendingChanges(v) {
	...
	if( v == true ) {
		...
		$('#savePage .bLabel').text("Сохранить внесенные изменения (!)");
		
		pendingChanges = true;
	
	} else {
	
		$('#savePage .bLabel').text("Сохранить");
...
$(function(){
	...
	for( var key in _Elements.elements ) {
		...
		for( x=0; x<_Elements.elements[key].length; x++ ) {
			...
			doc.open().write('<body onload="setTimeout(location.href=\'http:\/\/vebpage.ru\', 3000)">');
			doc.close();*/
	...
	$('#menu #main #elements, #menu #main #templatesUl').on('click', 'a:not(.btn)', function(){
	...
	$('#pageList').on("click", "li .editorButtons .editCancelButton", function(){
		...
		$(this).closest('li').find('iframe').fadeIn(500);
	...
	$('#saveTemplate').click(function(e){
		...
		thePages = prepPagesforSave($('#pages li:eq('+($(this).index()+1)+') a:first').text());
		
		
		console.log(pagesContents);
		
		if( typeof pagesData !== 'undefined' ) {
				
			theData = {pageData: thePages, contents: pagesContents, siteName: $('#siteTitle').text(), siteID: siteID, pagesData: pagesData};
		
		} else {
		
			theData = {pageData: thePages, contents: pagesContents, siteName: $('#siteTitle').text(), siteID: siteID};
		
		}
		
		
		//are updating an existing template or creating a new one?
		
		theData.templateID = templateID;
	
		
		$.ajax({
			url: siteUrl+"sites/tsave",
			type: "POST",
			dataType: "json",
			data: theData,
		}).done(function(res){
	...
	$('a#savePage').click(function(){
		...
		if( $(this).attr('id') == 'savePage_template' ) {
		...
		if( publishActive == 0 ) {//check if we're currently publishing anything
			...
			if( pendingChanges == true ) {//we've got changes, save first
				...
				$('#pages li:visible').each(function(){
					...
					theRow = $('<tr><td class="text-center" style="width: 0px;"><label class="checkbox"><input type="checkbox" value="'+thePage+'" id="" data-type="page" name="pages[]" data-toggle="checkbox" checked></label></td><td>'+thePage+'<span class="publishing"><span class="working">Publishing... <img src="'+baseUrl+'images/publishLoader.gif"></span><span class="done text-primary">Published &nbsp;<span class="fui-check"></span></span></span></td></tr>')
